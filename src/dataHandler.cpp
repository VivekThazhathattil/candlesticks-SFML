...
void DataHandler::fetch(const std::string filePath, std::vector<std::string> &xData, 
	...
	while( getline(pFile, line) ){
		...
		bool isDouble = true; // whether std::stod (string to double) is valid for a substring
		...
		for(unsigned i = 0; i < v.size(); ++i){
			if(i !=0){ // i = 0 corresponds to xData and everything else is yData
				...
				try{
					...
					std::cerr << "Invalid argument in std::stod encountered. Omitting..." << std::endl;
					isDouble = false;
					break;
				}
				if(isDouble)
